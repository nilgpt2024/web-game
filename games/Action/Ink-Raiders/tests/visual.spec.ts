...
test('renders a nonblank interactive game canvas', async ({ page }, testInfo) => {
  ...
  // Click start button first - game only runs after this
  await page.locator('#start-btn').click();
  await expect(page.locator('#title-screen')).toHaveClass(/hidden/);

  // Wait for game to start rendering frames
  await page.waitForFunction(() => (window.__THREE_GAME_DIAGNOSTICS__?.frame ?? 0) > 10, { timeout: 30000 });
