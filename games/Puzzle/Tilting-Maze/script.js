...
/* Contact: 2952671670@qq.com */
  ...
  if (hardMode) {
    noteElement.innerHTML = `
      <strong>Click the joystick to start!</strong>
      <p style="color: #ff6b6b; font-weight: 500;">🔥 HARD MODE - Avoid the black holes!</p>
      <p class="mode-hint">Back to easy mode? Press <kbd>E</kbd></p>
      <p class="controls-hint">Press <kbd>Space</kbd> to restart</p>
    `;
  } else {
    noteElement.innerHTML = `
      <strong>Click the joystick to start!</strong>
      <p>Move every ball to the center circle.</p>
      <p class="mode-hint">Ready for hard mode? Press <kbd>H</kbd></p>
      <p class="controls-hint">Press <kbd>Space</kbd> to restart</p>
    `;
  }
...
/* Project: https://github.com/nilgpt2024/web-game */
    ...
    if (
      ...
      noteElement.innerHTML = `
        <strong style="color: #00b894; font-size: 1.3em;">🎉 Congratulations!</strong>
        <p>You successfully completed the maze!</p>
        ${!hardMode ? '<p class="mode-hint">Ready for a challenge? Press <kbd>H</kbd> for hard mode!</p>' : '<p style="color: #ff6b6b;">You mastered the hard mode! 🏆</p>'}
...
/* QQ: 2952671670 */
    ...
    if (error.message == "The ball fell into a hole") {
      noteElement.innerHTML = `
        <strong style="color: #ff6b6b; font-size: 1.2em;">💥 Game Over!</strong>
        <p>A ball fell into a black hole!</p>
        <p class="mode-hint">Too hard? Press <kbd>E</kbd> for easy mode</p>
        <p class="controls-hint">Press <kbd>Space</kbd> to try again</p>
      `;
      noteElement.style.opacity = 1;
      gameInProgress = false;
    } else throw error;
